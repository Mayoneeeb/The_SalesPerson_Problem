...
void printList() {
    ...
    for (int i = 0; i < numCities; i++) {
        ...
        for (int j = 0; j < (int)adjList[i].size(); j++)
            cout << cityNames[adjList[i][j].to] << "(" << adjList[i][j].weight << ") ";
        cout << "\n";
    }
}
...
int routeCost(const vector<int>& route) {
    ...
    cost += adjMatrix[route.back()][route[0]]; // return to start
    return cost;
}
...
vector<int> bruteForceTSP(int startCity) {
    ...
    do {
        ...
        // Skip permutations that use missing edges
        bool valid = true;
        for (int i = 0; i < (int)route.size() - 1; i++) {
            if (adjMatrix[route[i]][route[i + 1]] == 0) { valid = false; break; }
        }
        if (adjMatrix[route.back()][route[0]] == 0) valid = false;
        if (!valid) continue;
...
// ============================================================
//  Algorithm 2 — Greedy (Nearest Neighbor) TSP
// ============================================================
vector<int> greedyTSP(int startCity) {
    ...
    for (int step = 1; step < numCities; step++) {
        int nearest = -1, minDist = 999999999;
        for (int j = 0; j < numCities; j++) {
...
void dijkstraShortestPath() {
    ...
    cin >> src;
    if (src < 0 || src >= numCities) { cout << "Invalid source.\n"; return; }

    cout << "Enter destination city index: ";
    cin >> dst;
    if (dst < 0 || dst >= numCities) { cout << "Invalid destination.\n"; return; }

    if (src == dst) { cout << "Source and destination are the same. Cost = 0.\n"; return; }
    ...
    if (dist[dst] == INF) {
        cout << "\nNo path found from " << cityNames[src] << " to " << cityNames[dst] << ".\n";
        return;
    }
    ...
    cout << "From: " << cityNames[src] << "  To: " << cityNames[dst] << "\n";
    cout << "Path: ";
...
void runAlgorithm(int choice, int startCity) {
    ...
    if (choice == 1) {
        ...
        if (route.empty()) {
            cout << "\n=== Brute Force ===\n";
            cout << "No complete route found — make sure all cities are connected.\n";
            return;
        }
    }
    else if (choice == 2) {
...
void compareAlgorithms(int startCity) {
    ...
    if (route1.empty())
        cout << "Brute Force  ->  No valid complete route found  |  Matrix (Static)\n";
    else
        cout << "Brute Force  ->  Cost: " << routeCost(route1) << "  |  Matrix (Static)\n";

    vector<int> route2 = greedyTSP(startCity);
    cout << "Greedy       ->  Cost: " << routeCost(route2) << "  |  List (Dynamic)\n";
...
bool handleEdgeCases() {
    ...
    if (numCities == 1) {
        cout << "Edge Case: Only 1 city. No route needed. Cost = 0.\n";
        return true;
    }
    for (int i = 0; i < numCities; i++)
        for (int j = 0; j < numCities; j++)
            if (i != j && adjMatrix[i][j] == 0)
                cout << "Warning: Missing edge (" << cityNames[i] << " -> " << cityNames[j] << " = 0).\n";
    return false;
}
...
int main() {
    ...
    string filename;
    cout << "Enter graph file name (e.g. cities.txt): ";
    cin >> filename;

    if (!loadGraph(filename)) {
        cout << "Using built-in example (A B C D)...\n";
    ...
    int startCity = 0;
    cout << "\nEnter starting city index (0 = " << cityNames[0] << "): ";
    cin >> startCity;
    ...
    do {
        ...
        cout << "2. Greedy (Nearest Neighbor) TSP\n";
        cout << "3. Compare Both Algorithms\n";
        cout << "4. Find Shortest Path Between Two Cities (Dijkstra)\n";
        cout << "0. Exit\n";
        cout << "Choice: ";
        cin >> choice;
